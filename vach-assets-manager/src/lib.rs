            ...
            match entry {
                Ok(entry) => {
                    ...
                    if !extensions.iter().any(|ext| &path_ext == ext) {
                        ...
                        log::trace!("Skipping file {}, reason: wrong file extension", &abs_path);
                        continue;
                    }
                    ...
                    let relative_path = match pathdiff::diff_paths(entry.path(), base_path) {
                        Some(p) => p,
                        None => {
                            log::warn!("Failed to make path relative, skipping '{}'. (base path: '{}')", &abs_path, base_path);
                            continue;
                        }
                    };
...
impl Source for ArchiveSource {
    fn read(&self, id: &str, ext: &str) -> std::io::Result<Cow<[u8]>> {
        ...
        let archive = match self.archives.get(&archive_name) {
            ...
            None => return Err(std::io::Error::new(std::io::ErrorKind::NotFound, format!("Archive '{}' not found", archive_name))),
        };
    ...
    fn read_dir(&self, id: &str, f: &mut dyn FnMut(DirEntry)) -> std::io::Result<()> {
        Ok(self.archives.keys().filter(|val| val.starts_with(id)).map(|val| {
    ...
    fn exists(&self, entry: DirEntry) -> bool {
        match entry {
            ...
            DirEntry::Directory(path) => {
                self.archives.keys().any(|val| val.starts_with(path))
            },
        }
    }
}
...
fn split_collect_all_except_last(string: &str, split: char) -> String {
    ...
    let mut string = result.iter().map(|val| val.to_string()).map(|mut val| { val.push(split); val }).collect::<String>();
...
fn test_id_and_extension_to_file_archive_name() {
    assert_eq!(id_and_extension_to_file_archive_name("music.ambient", "flac"), (
        "music".to_string(), "ambient.flac".to_string()
    ));

    assert_eq!(id_and_extension_to_file_archive_name("music.forest.ambient", "wav"), (
        "music.forest".to_string(), "ambient.wav".to_string()
    ));

    assert_eq!(id_and_extension_to_file_archive_name("shared.music.forest.ambient", "wav"), (
