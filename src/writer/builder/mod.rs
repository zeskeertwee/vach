...
impl<T: Read> Builder<T> {
    ...
    pub fn write<W: Write>( &mut self, target: &mut W, config: &BuilderConfig ) -> anyhow::Result<()> {
        ...
        target.write_all(&config.header_config.magic)?;
        ...
        target.write_all(&flags_temp.bits().to_le_bytes())?;

        target.write_all(&config.header_config.minimum_version.to_le_bytes())?;
        ...
        for (path, leaf) in self.leafs.iter_mut() {
            ...
            if leaf.config.sign {
                entry.signature = Some(
                    config
                        ...
                        .as_ref()
                        .ok_or(anyhow::anyhow!(
                            "No keypair provided, yet leaf-config requires the leaf be signed"
                        ))?
                        .sign(&glob),
                );
...

        {
            ...
            match &config.keypair {
                ...
                None => {
                    target.write_all(&[0x53u8; esdalek::SIGNATURE_LENGTH])?;
                }
            }
        }
